...
void main() {
    ...
    float ffc2  = -1. + 2.*pow(smoothstep(0., 1., fff(uv*vec2(1., 18.)*1.21+seed*2.67+vec2(ffb1, ffb2), seed*1.23+7.56)), 3.);
    ...
    float ooi = .1 + .9*pow(1.-abs(uv.y-.5)/.5, 2.);

    vec2 udi = vec2(ffc1*.1, ffc2);
    //udi = uDir;

    vec3 colorr = gaussianBlur(tDiffuse, uv + ooi*ffc*.08*udi, 5.*ooi*ffc*4.*1.40*amp/1000.*udi ); //*uu*randomNoise(uv)
    vec3 colorg = gaussianBlur(tDiffuse, uv + ooi*0.*ffc*.08*udi, 5.*ooi*ffc*4.*1.40*amp/1000.*udi ); //*uu*randomNoise(uv)
    vec3 colorb = gaussianBlur(tDiffuse, uv - ooi*ffc*.08*udi, 5.*ooi*ffc*4.*1.40*amp/1000.*udi ); //*uu*randomNoise(uv)
    vec3 color = vec3(colorr.r, colorg.g, colorb.b);
    gl_FragColor = vec4(vec3(uu.xy, 0.), 1.);
    gl_FragColor = vec4(vec3(uu*randomNoise(uv), 0.), 1.);
