...
void main() {
    ...
    float oof = -1.+2.*smoothstep(0., 1., fff(vUV.xy*vec2(13., 1.)*2.5+4.314*14.+seed*13.11 + .021*vec2(oofx, oofy*20.), 6.314+seed*3.12));
    oof = .01+(-.01+1.)*(oof/2.+.5);
    // oof *= (.5+.5*oo);
    ...
    //vec3 mixed = mix(irid, col, oo*oof+(1.-oo)*1.);
    vec3 mixed = mix(colors(mod(vUV.x*1.,1.)), colors(mod(vUV.x*1.+.24,1.)), oof);
    ...
    float didi = smoothstep(0.0, she, vUV.y) * (1. - smoothstep(1.0-she, 1.0, vUV.y));
