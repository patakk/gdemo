...
void main() {
    ...
    float ffb1 = -1. + 2.*pow(smoothstep(0., 1., fff(uv*.1*vec2(1., 1.00)*.14+seed1*14.+vec2(ffa1, ffa2), seed1+11.13)), 1.);
    float ffb2 = -1. + 2.*pow(smoothstep(0., 1., fff(uv*.1*vec2(1., 1.00)*.14+seed1*1.42+vec2(ffa1, ffa2), seed1+33.43)), 1.);
    float ffc  = -1. + 2.*pow(smoothstep(0., 1., fff(uv*.1*vec2(1., 1.00)*1.77+seed1*2.67+vec2(ffa1, ffa2), seed1+73.77)), 3.);
    float ffc1  = -1. + 2.*pow(smoothstep(0., 1., fff(uv*.1*vec2(13., 1.00)*33.43+seed1*3.6877+10.*vec2(ffb1, ffb2), seed1+2.53)), 3.);
    float ffc2  = -1. + 2.*pow(smoothstep(0., 1., fff(uv*.1*vec2(13., 1.00)*33.21+seed1*2.67+10.*vec2(ffb1, ffb2), seed1+7.56)), 3.);
    ...
    vec2 uvm = uv + 4.*vec2(ffc1, ffc2)/resolution.x;
    vec4 texel = texture2D( tDiffuse, uv);
    vec4 texelDepth = texture2D( tDiffuse2, uv);
    vec4 texelBlurDepth = texture2D( tDiffuse3, uv);
    vec4 texelBlur = texture2D( tDiffuse4, uv);
    ...
    salt2 = random(uv+randomNoise(uv));
    ...
    //float fffa = fbm(uvv + vec2(ffa, ffb));


    gl_FragColor = vec4(vec3(salt5), 1.);
    ...
    gl_FragColor = vec4(texelBlur.rgb, 1.);
    gl_FragColor = vec4(res.rgb, 1.);
    gl_FragColor = vec4(texelBlurDepth.rgb, 1.);
    gl_FragColor = vec4(texelBlur.rgb, 1.);
    gl_FragColor = vec4(vec3( .2+.8*smoothstep(0.2, .6, fff(uvm*3.2, 31.3112+seed1*21.3)) ), 1.);
    gl_FragColor = vec4(texelDepth.rgb, 1.);
    gl_FragColor = vec4(texel.rgb, 1.);
    gl_FragColor = vec4(vec3(ffc1), 1.);
    gl_FragColor = vec4(texelBlur.rgb, 1.);
    gl_FragColor = vec4(texel.rgb, 1.);
    gl_FragColor = vec4(vec3(salt5), 1.);
    gl_FragColor = vec4(res.rgb, 1.);
